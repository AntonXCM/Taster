...
{
    public class HealingCombinations : MonoBehaviour
    {
        ...
        {
            int x = 0, y = 0;
            for (int i = 0; i < Database.HealingCombinations.Count; i++)
            {
                BookCombination currentItem = Instantiate(bookCombinationPrefab, transform);
                currentItem.Combination = Database.HealingCombinations[i];
